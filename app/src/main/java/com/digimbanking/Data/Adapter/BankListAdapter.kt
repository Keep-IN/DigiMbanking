...
class BankListAdapter: RecyclerView.Adapter<BankListAdapter.ViewHolder>() {
    private var itemListener: ((DataBank) -> Unit)? = null
    private val data: MutableList<DataBank> = mutableListOf()
    ...
    fun submitList(list: List<DataBank>) {
    ...
    inner class ViewHolder(private val binding: BankListViewBinding): RecyclerView.ViewHolder(binding.root) {
        fun setData(item: DataBank, listener: ((DataBank) -> Unit)?){
            binding.root.setOnClickListener{
                listener?.invoke(item)
            }
            binding.tvNamaBank.text = item.namaBank
        }
    }

    fun setOnclickItem(listener: ((DataBank) -> Unit)?){
